 ...
 * @version 1.0.2 - Option A - Fixed offsetParent bug for position:fixed elements
 */
    ...
    const OptionAEnhancer = {
        ...
        waitForBanner: function() {
            return new Promise((resolve) => {
                ...
                // Fixed: Don't use offsetParent for fixed position elements
                if (existing && (existing.offsetParent !== null || getComputedStyle(existing).position === 'fixed')) {
                    resolve(existing);
                    return;
                }
                ...
                const observer = new MutationObserver((mutations) => {
                    ...
                    // Fixed: Don't use offsetParent for fixed position elements
                    if (banner && (banner.offsetParent !== null || getComputedStyle(banner).position === 'fixed')) {
                        observer.disconnect();
                        resolve(banner);
                    }
                });
